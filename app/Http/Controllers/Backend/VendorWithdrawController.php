...
class VendorWithdrawController extends Controller
{
    ...
    public function index(VendorWithdrawDataTable $dataTable)
    {
        ...
        $totalWithdrawCharge = WithdrawRequest::where('status', 'paid')->sum(DB::raw('total_amount + withdraw_charge'));


        $currentBalance = $totalEarnings - $totalWithdrawCharge;
        $totalWithdraw = WithdrawRequest::where('status', 'paid')->sum(DB::raw('total_amount'));
